...
export function DocumentPreview({ sessionId, file, colorMode = "BW", sides = "SINGLE", selectedPagesCount }: DocumentPreviewProps) {
  ...
  const [viewerProvider, setViewerProvider] = useState<"google" | "office">("google");
  ...
  return (
    <>
      <div className="mt-4 rounded-card border border-slate-200 bg-white p-3.5 sm:p-4 shadow-soft">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2 font-display text-sm font-bold text-ink">
            ...
            <span className="truncate max-w-[170px] sm:max-w-[240px]">Document Preview</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[11px] sm:text-xs font-bold px-2 sm:px-2.5 py-0.5 rounded-full bg-primary-light text-primary-dark uppercase">
        ...
        <div className="mt-3 relative rounded-control bg-slate-50 border border-slate-200 overflow-hidden flex items-center justify-center min-h-[180px] max-h-[280px] p-2.5">
          ...
          {isImage && (
            <img
              ...
              className="max-h-[260px] w-auto object-contain rounded-lg cursor-pointer hover:opacity-95 transition-opacity shadow-sm"
              onClick={() => setFullscreen(true)}
          ...
          {isPdf && !pdfRenderError && (
            <div className="overflow-auto max-h-[260px] w-full flex justify-center">
              <canvas
                ref={canvasRef}
                onClick={() => setFullscreen(true)}
          ...
          {/* PDF Fallback Frame */}
          {isPdf && pdfRenderError && (
            <object data={previewUrl} type="application/pdf" className="w-full h-[220px] rounded">
              <iframe src={`${previewUrl}#toolbar=0`} title={file.originalName} className="w-full h-[220px] border-0" />
            </object>
          )}
          ...
          {isPptx && (
            <div
              onClick={() => setFullscreen(true)}
              className="w-full max-w-sm aspect-[16/9] bg-slate-900 text-white rounded-xl p-3.5 shadow-lg border-2 border-slate-700 flex flex-col justify-between cursor-pointer hover:border-amber-400 transition-all relative overflow-hidden group"
            >
              ...
              <div className="my-auto space-y-2">
                <p className="font-display font-bold text-xs sm:text-sm text-slate-100 truncate">{file.originalName.replace(/\.[^/.]+$/, "")}</p>
              ...
              <div className="flex justify-between items-center text-[10px] text-slate-400 pt-2 border-t border-slate-800">
                ...
                <span className="text-amber-400 font-bold group-hover:underline flex items-center gap-1">
                  <Maximize2 size={12} /> Open Full Preview
                </span>
              </div>
            </div>
          )}
          ...
          {isExcel && !isText && (
            <div
              onClick={() => setFullscreen(true)}
              className="w-full h-[200px] bg-white rounded-xl p-3 border-2 border-emerald-500/40 shadow-sm cursor-pointer overflow-hidden flex flex-col justify-between group"
            >
              ...
              <div className="text-[10px] text-emerald-700 font-bold text-center group-hover:underline flex items-center justify-center gap-1">
                <Maximize2 size={12} /> Open Full Web Preview
              </div>
            </div>
          )}
          ...
          {isDocx && (
            <div
              onClick={() => setFullscreen(true)}
              className="w-44 sm:w-48 h-[200px] bg-white rounded-lg p-3.5 shadow-md border-2 border-blue-200 flex flex-col justify-between cursor-pointer hover:border-blue-500 transition-all group relative"
            >
              ...
              <div className="text-[10px] text-blue-600 font-bold text-center pt-2 border-t border-slate-100 group-hover:underline flex items-center justify-center gap-1">
                <Maximize2 size={12} /> {file.pageCount} Pages · Preview
              </div>
            </div>
          )}
          ...
          {isText && textContent && (
            <div
              onClick={() => setFullscreen(true)}
              className="w-full h-[200px] bg-white p-3 rounded border border-slate-200 text-xs font-mono text-slate-800 overflow-auto whitespace-pre-wrap cursor-pointer"
            >
              {textContent}
            </div>
          )}
          ...
          {!isImage && !isPdf && !isPptx && !isExcel && !isDocx && (!isText || !textContent) && (
            <div onClick={() => setFullscreen(true)} className="p-5 text-center bg-white rounded-control border border-slate-200 w-full shadow-sm cursor-pointer">
              <div className="w-12 h-12 mx-auto rounded-full bg-primary-light flex items-center justify-center mb-2">
                <FileText size={24} className="text-primary" />
              </div>
              <p className="text-xs sm:text-sm font-bold text-ink truncate max-w-[220px] mx-auto">{file.originalName}</p>
              ...
              <span className="inline-block mt-2.5 px-3 py-1 bg-slate-100 text-slate-700 text-[10px] font-bold rounded-full">
        ...
        {/* Metadata Badges */}
        <div className="mt-3 grid grid-cols-4 gap-1 pt-2 border-t border-slate-100 text-center">
      ...
      {/* Ultra-Responsive Fullscreen Document Preview Modal */}
      {fullscreen && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex flex-col w-screen h-[100dvh] overflow-hidden">
          {/* Responsive Header */}
          <div className="flex flex-wrap sm:flex-nowrap items-center justify-between text-white px-3 sm:px-4 py-2.5 sm:py-3 border-b border-white/10 bg-slate-950 shrink-0 gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <span className="font-display font-bold text-xs sm:text-sm truncate max-w-[160px] sm:max-w-xs">{file.originalName}</span>
              <span className="text-[10px] sm:text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 font-bold border border-emerald-500/30 shrink-0">
                {file.pageCount} Pgs
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-auto">
              {(isDocx || isPptx || isExcel) && (
                <div className="flex items-center bg-slate-900 rounded-lg p-0.5 border border-slate-800 text-[11px] sm:text-xs font-bold">
                  <button
          ...
          <div className="flex-1 w-full h-full p-1 sm:p-3 bg-slate-900/90 overflow-hidden flex items-center justify-center relative">
            {/* 1. Image Fullscreen */}
            {isImage && (
            ...
            {(isDocx || isPptx || isExcel) && (
              <div className="w-full h-full flex flex-col rounded-lg bg-white shadow-2xl overflow-hidden relative">
                ...
                {/* Responsive Bottom Info Switcher Bar */}
                <div className="p-1.5 sm:p-2 bg-slate-100 border-t border-slate-200 flex items-center justify-between text-[10px] sm:text-xs text-slate-600 shrink-0">
                  <span className="truncate pr-2">Official document web preview engine</span>
                  <button
                    onClick={() => setViewerProvider((p) => (p === "google" ? "office" : "google"))}
                    className="flex items-center gap-1 font-bold text-primary hover:underline shrink-0"
                  >
                    <RefreshCw size={11} /> Switch Viewer Engine
                  </button>
                </div>
              </div>
            )}
            ...
            {isText && textContent && (
              <div className="w-full max-w-3xl h-full bg-slate-950 text-emerald-400 rounded-xl p-4 sm:p-6 shadow-2xl border border-slate-800 font-mono text-[11px] sm:text-xs overflow-auto whitespace-pre-wrap leading-relaxed">
                {textContent}
              </div>
            )}
